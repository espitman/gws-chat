// This file generated automatically by gRPC gateway generator
// Generated at: 2024-04-14 21:17:44

package main

import userpb "github.com/espitman/gws-chat/pkg/protos/protogen/user-service"

type userServiceV1GetResponseDto struct {
	ResponseDto
	Payload userpb.V1GetResponse
}

type userServiceV1GetAllResponseDto struct {
	ResponseDto
	Payload userpb.V1GetAllResponse
}

type userServiceV1GetByIDsResponseDto struct {
	ResponseDto
	Payload userpb.V1GetByIDsResponse
}

type userServiceV1LoginResponseDto struct {
	ResponseDto
	Payload userpb.V1LoginResponse
}

type userServiceV1SetOnlineResponseDto struct {
	ResponseDto
	Payload userpb.V1SetOnlineResponse
}
